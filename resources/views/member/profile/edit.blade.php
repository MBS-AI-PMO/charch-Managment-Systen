        ...
        <div class="card p-6 md:p-8">
            ...
            <form method="POST" action="{{ route('member.profile.update') }}" class="space-y-5">
                ...
                <div class="flex items-center gap-4">
                    <div class="w-20 h-20 rounded-full bg-brand-primary/10 text-brand-primary flex items-center justify-center font-serif text-2xl">{{ $initials ?: 'M' }}</div>
                    <div>
                        <button type="button" class="btn-ghost text-sm" disabled>Change photo</button>
                        <p class="text-xs text-ink-muted mt-1">Avatar uploads coming soon.</p>
                    </div>
                    <input type="hidden" name="avatar_path" value="{{ old('avatar_path', $user->avatar_path) }}" />
                </div>
